...
// 3c çözümü:
ucetambolunenlerintoplami = ucetambolunenler.reduce(
  ...
  for (let sayi in sayac) {
    if (sayac[sayi] > 1) {
      tekraredensayilar.push(
        `${sayi} sayısı ${sayac[sayi]} kere tekrar edilmistir`
      );
    }
  }
  return tekraredensayilar;
}
console(tekrarSayisi(sayilar));
